...
for i=1:size(eta_choice,2)
  ...
  %alpha=alpha_choice(j);
    ...
    parfor trials=1:try_number
        ...
        TY_temp=TY{trials};
        Y_temp=Y{trials};
        [T_RF,R_filter_RM, R_filterSD_RM, R_filter_RM_div, R_filter_RM_final_distribution, R_filter_RM_particles]=particle_filter(TY_temp, Y_temp, M, 'Tau', 'RPF_NS', eta, alpha);
        add_error=0;
        ...
        for index=1:size(K_temp,1)
            entropy_temp(index)= entropy(R_filter_RM_final_distribution(index,:)/2);
        end
